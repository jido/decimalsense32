...
int main(int n, char * args[]) {
    ...
    printf(" -3.0e-2 / 0.006e-48 = %s\n", numberAsString32(div32(makeNumber32(-3, 0, -2), 6000)));
    printf(" 9.999999e-48 / 1.0e-54 = %s\n", numberAsString32(div32(makeNumber32(9, 9999990, -48), 1)));
    ...
    */
    static uint64_t powersOf2[340] = { // First non-zero digits of powers of 2 in range (-180 to 160) - 52 
    ...
    static int expo10[340] = { // base 10 exponent for powers of 2 in range (-180 to 160) - 52
        ...
        -9, -9, -9, -8, -8, -8, -7, -7, -7, -7, -6, -6, -6, -5, -5, -5,
    ...
    printf("3.437519e22 = %.8g = %s\n", (3 + 4375190 * 0.0000001) * ef[22 + 54], numberAsString32(makeNumber32(3, 4375190, 22)));
    long s = 90000790;
    ...
    for (int expn = -48; expn <= 47; ++expn)
    {
        for (int units = -9; units <= 9 - 5 * (expn == 47); units += 1 + (expn > -48 && units == -1))
        {
            ...
            for (int deci = 0; deci < 10000000; deci += 3 + r % 8)
            {
                ...
                int ee = (*((uint64_t*)&f) >> 52) & 0x7FF;
                uint64_t mm = (1LL << 52) + (*((uint64_t*)&f) & 0xFffffFFFFffff);
                uint32_t x = (powersOf2[ee + 180 - 1023] * ((mm + 500000) / 1000000) + 5000000000) / 10000000000;
                decims32 num = makeNumber32(units, deci, expn); //div32(s * r, );, s * r
                if ((r & 0xff764) == 0x2d524) {
                    printf(" %s=%.8g ", numberAsString32(num), f);
                    int e = expo10[ee + 180 - 1023] + 15;
                    if (x >= 100000000) {
                        x /= 10;
                        ++e;
                    }
                    printf("Decomposition: %u exp %d (%d)\n", x, e, ee - 1023);
                }
                r >>= 3;
                if (r == 0) r = random();
            }
